...
int main() {
    ...
    std::string filename = "C:/Programming/LAB_8/surnames.txt";
    ...
    int hammerPrice = t.get("Брус");
    std::cout << "Брус стоит: " << hammerPrice << std::endl; 

    // Удаляем элемент из таблицы
    t.remove("Песок");

    // Проверяем, есть ли в таблице элемент 
    bool examination = t.get("Песок") != -1;
    std::cout << "Наличие песка: " << (examination ? "есть" : "нету") << std::endl;  
    ...
    std::cout<<"\nExersize 3\n";
    std::string file_name = "C:/Programming/Lab8_Programming/photo.jpg";
    ...
    std::ofstream file("C:/Programming/Lab8_Programming/answer.txt");
    if (!file) {
